...
function mergeSameChannel(rows, windowSec){
  ...
  for (const s of sensors) {
    // find closest GPS within window
    ...
    if (best && Number.isFinite(best.lat) && Number.isFinite(best.lon)) {
      ...
      if (prevLat != null && prevLon != null && prevTime != null) {
        ...
        // Optional: ignore tiny jitter when stopped (e.g., <10 m)
        // if (segKm < 0.01) segKm = 0;
      ...
      out.push({
        ...
        speed_kmh: (speed ?? ''),
        seg_km: (segKm != null ? +segKm.toFixed(3) : ''),   // distance since previous merged row
        total_km: +totalKm.toFixed(3),                      // cumulative distance
        aqi: aqi ?? ''
      });
